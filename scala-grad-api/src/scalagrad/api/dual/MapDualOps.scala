...
trait MapDualOps[
    ...
    extension (m: DualMatrix)
        @targetName("elementWiseOpM_Op_map2")
        def map2(op: PScalar => PScalar, dOp: PScalar => PScalar): DualMatrix = m.mapElements2(op, dOp)
        @targetName("elementWiseOpM_Op2")
        def mapElements2(op: PScalar => PScalar, dOp: PScalar => PScalar): DualMatrix = elementWiseOpM(m, op, dOp)
        @targetName("columnWiseOpM_Op2")
        def mapColumns2(op: PColumnVector => PColumnVector, dOp: PColumnVector => PColumnVector): DualMatrix = columnWiseOpM(m, op, dOp)
        @targetName("rowWiseOpM_Op2")
        def mapRows2(op: PRowVector => PRowVector, dOp: PRowVector => PRowVector): DualMatrix = rowWiseOpM(m, op, dOp)

    extension (cv: DualColumnVector)
        @targetName("elementWiseOpCV_Op_map2")
        def map2(op: PScalar => PScalar, dOp: PScalar => PScalar): DualColumnVector = cv.mapElements2(op, dOp)
        @targetName("elementWiseOpCV_Op2")
        def mapElements2(op: PScalar => PScalar, dOp: PScalar => PScalar): DualColumnVector = elementWiseOpCV(cv, op, dOp)

    extension (rv: DualRowVector)
        @targetName("elementWiseOpRV_Op_map2")
        def map2(op: PScalar => PScalar, dOp: PScalar => PScalar): DualRowVector = rv.mapElements2(op, dOp)
        @targetName("elementWiseOpRV_Op2")
        def mapElements2(op: PScalar => PScalar, dOp: PScalar => PScalar): DualRowVector = elementWiseOpRV(rv, op, dOp)

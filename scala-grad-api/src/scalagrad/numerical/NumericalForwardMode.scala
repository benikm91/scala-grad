...
object NumericalForwardMode:
    ...
    def derive(
        ...
        alg => x =>
            approxScalar2ScalarWith(alg, 1e-6)(f(alg))(x)
    ...
    def derive(
        ...
        alg => x =>
            approxColumnVector2ScalarWith(alg, 1e-6)(f(alg))(x)
    ...
    def derive(
        ...
        alg => x =>
            approxRowVector2ScalarWith(alg, 1e-6)(f(alg))(x)
    ...
    def derive(
        ...
        alg => x =>
            approxMatrix2ScalarWith(alg, 1e-6)(f(alg))(x)
    ...
    def derive(
        ...
        alg => (s1, s2) =>
            approxTuple2ScalarWith(alg, 1e-6)(f(alg).tupled)((s1, s2))
    ...
    def derive(
        ...
        alg => (cv, s) =>
            approxTuple2ScalarWith(alg, 1e-6)(f(alg).tupled)((cv, s))
    ...
    def derive(
        ...
        alg => (cv1, cv2) =>
            approxTuple2ScalarWith(alg, 1e-6)(f(alg).tupled)((cv1, cv2))
    ...
    def derive(
        ...
        alg => (m1, m2) =>
            approxTuple2ScalarWith(alg, 1e-6)(f(alg).tupled)((m1, m2))
    ...
    def derive(
        ...
        alg => (m, cv) =>
            approxTuple2ScalarWith(alg, 1e-6)(f(alg).tupled)((m, cv))
    ...
    def derive(
        ...
        alg => (m, rv) =>
            approxTuple2ScalarWith(alg, 1e-6)(f(alg).tupled)((m, rv))
    ...
    def derive(
        ...
        alg => (m, s) =>
            approxTuple2ScalarWith(alg, 1e-6)(f(alg).tupled)((m, s))
    ...
    def approxScalar2ScalarWith(alg: MatrixAlgebraDSL, epsilon: Double)(f: alg.Scalar => alg.Scalar): alg.Scalar => alg.Scalar = s =>
            approxScalar(alg)(s, alg.lift(epsilon), f)

    def approxColumnVector2ScalarWith(alg: MatrixAlgebraDSL, epsilon: Double)(f: alg.ColumnVector => alg.Scalar): alg.ColumnVector => alg.ColumnVector = cv =>
            approxColumnVector(alg)(cv, alg.lift(epsilon), f)

    def approxRowVector2ScalarWith(alg: MatrixAlgebraDSL, epsilon: Double)(f: alg.RowVector => alg.Scalar): alg.RowVector => alg.RowVector = rv =>
            approxRowVector(alg)(rv, alg.lift(epsilon), f)

    def approxMatrix2ScalarWith(alg: MatrixAlgebraDSL, epsilon: Double)(f: alg.Matrix => alg.Scalar): alg.Matrix => alg.Matrix = m =>
            approxMatrix(alg)(m, alg.lift(epsilon), f)

    def approxTuple2ScalarWith[T <: Tuple](alg: MatrixAlgebraDSL, epsilon: Double)(f: T => alg.Scalar): T => T = t =>
